...
def generator(map_proccess, mapping):
    ...
    try:
        ...
        with open(f"result-{map_proccess.uuid}.owl", "w") as owl_file:
            owl_file.write(res_n.serialize(format='turtle').decode("utf-8"))
            
        # temp_onto = get_ontology(f"file://result-{map_proccess.uuid}.owl").load()
        # onto_classes = list(temp_onto.classes())
        # list_mapped_elements = [i.iri for i in onto_classes]
        return True 
    except:
        return False